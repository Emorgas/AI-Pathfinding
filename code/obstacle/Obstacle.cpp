...
{
	glPushMatrix();
		glTranslatef(m_CurrentPosition.x + m_Offset.x + NUDGE, m_fY, m_CurrentPosition.z - m_Offset.z - NUDGE);
...
position Obstacle::GetLogicalPosition()
{
	position tempPosition;

	tempPosition.x = m_CurrentPosition.x + m_Offset.x;// -NUDGE;
	tempPosition.z = m_CurrentPosition.z - m_Offset.z;//+NUDGE;
